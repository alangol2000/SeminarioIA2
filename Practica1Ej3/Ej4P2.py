...
# Inicialización de los términos de momentum para los pesos
momentum_input_hidden = np.zeros((input_neurons, hidden_neurons))
momentum_hidden_output = np.zeros((hidden_neurons, output_neurons))
...
for epoch in range(epochs):
    ...
    # Retropropagación y ajuste de pesos con momentum
    d_output = error * sigmoid_derivative(output)
    error_hidden = d_output.dot(weights_hidden_output.T)
    d_hidden = error_hidden * sigmoid_derivative(output_hidden)
    
    # Cálculo de los términos de momentum
    momentum_input_hidden = momentum * momentum_input_hidden + X.T.dot(d_hidden) * learning_rate
    momentum_hidden_output = momentum * momentum_hidden_output + output_hidden.T.dot(d_output) * learning_rate
